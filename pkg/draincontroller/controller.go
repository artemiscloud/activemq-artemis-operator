...
func (c *Controller) AddInstance(instance *brokerv1beta1.ActiveMQArtemisScaledown) {
	...
	c.log.Info("adding a new scaledown instance", "key", namespacedName)
	c.ssNamesMap[namespacedName] = instance.Annotations
	c.log.V(1).Info("Added new instance", "key", namespacedName, "now values", len(c.ssNamesMap))
...
func (c *Controller) Run(threadiness int) error {
	...
	c.log.Info("Starting StatefulSet scaledown cleanup controller")

	// Wait for the caches to be synced before starting workers
	c.log.V(1).Info("Waiting for informer caches to sync")
	if ok := cache.WaitForCacheSync(c.stopCh, c.statefulSetsSynced, c.podsSynced); !ok {
		return fmt.Errorf("failed to wait for caches to sync")
	}

	c.log.V(1).Info("Starting workers")
	...
	<-c.stopCh
	c.log.V(1).Info("Shutting down workers")
...
func (c *Controller) processNextWorkItem() bool {
	c.log.V(1).Info("Processing next work item")
	obj, shutdown := c.workqueue.Get()

	if shutdown {
		return false
	}
	c.log.V(1).Info("Got object from queue", "obj", obj)
	...
	err := func(obj interface{}) error {
		...
		if key, ok = obj.(string); !ok {
			c.log.V(1).Info("invalid obj, forget it", "obj", obj)
		...
		c.log.V(1).Info("calling syncHandler to process this one", "key", key)
		...
		c.workqueue.Forget(obj)
		c.log.V(1).Info("Successfully processed '" + key + "'")
		return nil
	}(obj)
...
func (c *Controller) syncHandler(key string) error {
	c.log.V(1).Info("SyncHandler invoked for " + key)
	...
	c.log.V(1).Info("got sts from lister", "namespace", namespace, "name", name, "error?", err)
	if err != nil {
...
func (c *Controller) processStatefulSet(sts *appsv1.StatefulSet) error {
	...
	c.log.V(1).Info("Processing statefulset", "sts", sts.Name)

	if *sts.Spec.Replicas == 0 {
		// Ensure data is not touched in the case of complete scaledown
		c.log.V(1).Info("Ignoring StatefulSet " + sts.Name + " because replicas set to 0.")
		return nil
	}

	c.log.V(1).Info("Statefulset " + sts.Name + " Spec.Replicas set to " + strconv.Itoa(int(*sts.Spec.Replicas)))

	if len(sts.Spec.VolumeClaimTemplates) == 0 {
		// nothing to do, as the stateful pods don't use any PVCs
		c.log.Info("Ignoring StatefulSet " + sts.Name + " because it does not use any PersistentVolumeClaims.")
		return nil
	}
	c.log.V(1).Info("Statefulset " + sts.Name + " Spec.VolumeClaimTemplates is " + strconv.Itoa((len(sts.Spec.VolumeClaimTemplates))))
	...
	c.log.V(1).Info("Looking through all the pods...")
	for _, ordinal := range ordinals {

		c.log.V(1).Info("looking ordinal", "ordinal", ordinal)
		if ordinal == 0 {
			// This assumes order on scale up and down is enforced, i.e. the system waits for n, n-1,... 2, 1 to scaledown before attempting 0
			c.log.V(1).Info("Ignoring ordinal 0 as no other pod to drain to.")
			continue
		}
		...
		c.log.V(1).Info("got pod name", "name", podName)
		...
		if isDrainPod(pod) {
			c.log.Info("Found a drain pod", "pod name", podName)
			...
			if sts.Spec.PodManagementPolicy == appsv1.OrderedReadyPodManagement {
				...
				c.log.V(1).Info("sts has orderReadyPodManagement policy, break")
				break
			}
		}
		...
		if int32(ordinal) >= *sts.Spec.Replicas {
			c.log.Info("ordinal is greater then replicas", "ordinal", ordinal, "replicas", *sts.Spec.Replicas)
			...
			if pod == nil { // TODO: what if the PVC doesn't exist here (or what if it's deleted just after we create the pod)
				c.log.Info("Found orphaned PVC(s) for ordinal " + strconv.Itoa(ordinal) + ". Creating drain pod " + podName)
				...
				if corev1.PodRunning != ordinalZeroPod.Status.Phase {
					c.log.V(1).Info("Ordinal zero pod " + sts.Name + " status phase not PodRunning, waiting for it to be Running.")
					continue
				}
				...
				for _, podCondition := range podConditions {
					if corev1.PodReady == podCondition.Type {
						if corev1.ConditionTrue != podCondition.Status {
							c.log.V(1).Info("Ordinal zero pod " + sts.Name + " podCondition Ready not True, waiting for it to True.")
						}
						if corev1.ConditionTrue == podCondition.Status {
							c.log.V(1).Info("Ordinal zero pod " + sts.Name + " podCondition Ready True, proceeding to create drainer pod.")
				...
				c.log.Info("Creating new drain pod...", "sts", sts)
				...
				c.log.V(1).Info("Now creating the drain pod in namespace "+sts.Namespace, "pod", pod)
...
func (c *Controller) getClaims(sts *appsv1.StatefulSet) (claimsGroupedByOrdinal map[int][]*corev1.PersistentVolumeClaim, err error) {
	...
	c.log.V(1).Info("getClaims allClaims", "len", len(allClaims))

	claimsMap := map[int][]*corev1.PersistentVolumeClaim{}
	for _, pvc := range allClaims {
		c.log.V(1).Info("getClaims allClaims pvc name is " + pvc.Name)
		if pvc.DeletionTimestamp != nil {
			c.log.V(1).Info("PVC " + pvc.Name + " is being deleted. Ignoring it.")
			continue
		}
...
func (c *Controller) createDrainRBACResources(namespace string) {
	c.log.Info("Creating drain pod rbac resources", "namespace", namespace)
...
func (c *Controller) cleanupDrainRBACResources(namespace string) {
	if !c.localOnly {
		c.log.V(1).Info("Cleaning up drain pod rbac resources", "namespace", namespace)
		...
		c.log.V(1).Info("Drain service account cleaned up", "namespace", namespace)
	}
}
...
func (c *Controller) cleanUpDrainPodIfNeeded(sts *appsv1.StatefulSet, pod *corev1.Pod, ordinal int) error {
	...
	case (corev1.PodSucceeded):
		c.log.Info("Drain pod " + podName + " finished.")
		if !c.localOnly {
		...
		for _, pvcTemplate := range sts.Spec.VolumeClaimTemplates {
			...
			c.log.Info("Deleting PVC " + pvcName)
		...
		//
		c.log.Info("Deleting drain pod " + podName)
	...
	case (corev1.PodFailed):
		c.log.Info("Drain pod " + podName + " failed.")
...
func (c *Controller) enqueueStatefulSet(obj interface{}) {
	...
	if enquequingSTS.ObjectMeta.Labels[selectors.LabelResourceKey] == "" {
		c.log.V(1).Info("Skipping statefulset without expected label", "key", key, "controller ns", c.name)
		return
	}

	c.log.V(1).Info("Enquequing statefulset", "key", key, "controller ns", c.name)
...
func (c *Controller) handlePod(obj interface{}) {
	...
	if object, ok = obj.(metav1.Object); !ok {
		...
		c.log.V(1).Info("Recovered deleted object " + object.GetName() + " from tombstone")
	}
	c.log.V(1).Info("Processing object: " + object.GetName())

	stsNameFromAnnotation := object.GetAnnotations()[AnnotationStatefulSet]
	if stsNameFromAnnotation != "" {
		c.log.V(1).Info("Found pod with " + AnnotationStatefulSet + " annotation pointing to StatefulSet " + stsNameFromAnnotation + ". Enqueueing StatefulSet.")
		sts, err := c.statefulSetLister.StatefulSets(object.GetNamespace()).Get(stsNameFromAnnotation)
		if err != nil {
			c.log.V(1).Info("Error retrieving StatefulSet " + stsNameFromAnnotation + ": " + err.Error())
			return
		}

		if *sts.Spec.Replicas == 0 {
			c.log.V(1).Info("NameFromAnnotation not enqueueing Statefulset " + sts.Name + " as Spec.Replicas is 0.")
			return
		}
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		if err != nil {
			c.log.V(1).Info("ignoring orphaned object " + object.GetSelfLink() + " of StatefulSet " + ownerRef.Name)
			return
		}

		if *sts.Spec.Replicas == 0 {
			c.log.V(1).Info("Name from ownerRef.Name not enqueueing Statefulset " + sts.Name + " as Spec.Replicas is 0.")
			return
		}
...
func (c *Controller) getClusterCredentials(namespace string, ssNames map[string]string) (string, string) {
	...
	c.log.V(1).Info("Try retrieving cluster credentials from secret", "secret", namespacedName)
	if err := resources.Retrieve(namespacedName, c.client, secretDefinition); err != nil {
		c.log.V(1).Info("Failed to retrieve cluster credentials from secret, using defaults", "err", err)
		return ssNames["CLUSTERUSER"], ssNames["CLUSTERPASS"]
	} else {
		c.log.V(1).Info("retrieved cluster credential from existing secret")
...
func (c *Controller) newPod(sts *appsv1.StatefulSet, ordinal int) (*corev1.Pod, error) {
	...
	c.log.Info("Creating newPod for ss", "ss", ssNamesKey)

	if _, ok := c.ssNamesMap[ssNamesKey]; !ok {
		c.log.V(1).Info("Cannot find drain pod data for statefule set", "namespace", ssNamesKey)
	...
	if c.localOnly {
		...
		c.log.Info("Setting drain pod service account", "service account name", DrainServiceAccountName)
