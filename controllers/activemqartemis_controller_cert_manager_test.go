	...
	Context("certutil functions", Label("check-cert-secret"), func() {
		...
		Context("certificate rotation", Label("certificate"), func() {
			...
			It("broker issuer certificate rotation", func() {
				...
				By("Updating issuer certificate: " + issuerCertName)
				Eventually(func(g Gomega) {
					g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: issuerCertName, Namespace: defaultNamespace}, &issuerCert)).Should(Succeed())
					issuerCert.Spec.DNSNames = []string{"after.artemiscloud.io"}
					g.Expect(k8sClient.Update(ctx, &issuerCert)).Should(Succeed())
				}, timeout, interval).Should(Succeed())
