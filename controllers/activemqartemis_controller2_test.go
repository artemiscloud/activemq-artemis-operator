...
var _ = Describe("artemis controller 2", func() {
	...
	Context("persistent volumes tests", Label("controller-2-test"), func() {
		It("controller sync period test", Label("controller-sync-period-test"), func() {
			defaultPeriod := common.GetRegularReconcileResyncPeriod()
			Expect(defaultPeriod).Should(Equal(common.DEFAULT_REGULAR_RESYNC_PERIOD))
			common.SetRegularReconcileResyncPeriod(2 * time.Second)
			...
			common.SetRegularReconcileResyncPeriod(common.DEFAULT_REGULAR_RESYNC_PERIOD)
